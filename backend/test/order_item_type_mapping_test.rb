...
module OnlineInvoice
  class OrderItemTypeFactoryTest < Minitest::Test
    ...
    def test_count_order_item_type_mapping
      product = Product.new('Clean Coders', 'membership')
      order_item_type_mapping = OrderItemTypeFactory.new(@customer, product)
      assert(order_item_type_mapping.types.keys.count == 4)
    end
