package main.challenge.strategies

import main.challenge.domain.*
import main.challenge.services.*
...
class PhysicalItemProcessingStrategy : OrderItemProcessingStrategy {
    override fun processOrderItem(orderItem: OrderItem, payment: Payment) {
        val shippingService = PhysicalShippingService(orderItem, payment)
...
class BookItemProcessingStrategy : OrderItemProcessingStrategy {
    override fun processOrderItem(orderItem: OrderItem, payment: Payment) {
        val shippingService = BookShippingService(orderItem, payment)
...
class DigitalItemProcessingStrategy : OrderItemProcessingStrategy {
    override fun processOrderItem(orderItem: OrderItem, payment: Payment) {
        val shippingService = DigitalShippingService(orderItem, payment)
